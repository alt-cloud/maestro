...
В данном документе описаны следующе варианты развворачивания образа: 
```
├── docker
│   ├── inet
│   └── localhost
...
- `inet` -  - вариант разворачивания на компьютере администратора или на сервере в локальной сети с доступом только как администратора, так и других возмможных пользователей по протоколу `https` через сетевой интерфейс.

## Reverse Proxy caddy для обеспечения доступа по протоколу https ид локальной и внешней сети
...
```
`Caddy` выполняет две фукнции:
- обеспечение доступа `web` и `backend` -интерфейсу по адресу `127.0.0.1:4466` `headlamp`, `API` интерфйесу maestro по адресу `127.0.0.1:5000`;
...
- Используйте **Docker (скрипт)** или **Podman**, если требуется максимальный контроль над параметрами запуска `run`, специфичные сетевые привилегии (CAP_NET_ADMIN) или если в вашей среде предпочтителен daemonless-подход (Podman). При использовании Podman не забудьте раскомментировать `--userns=keep-id` в скрипте для корректной работы с правами доступа к монтируемым томам.
...
на терминале, где запущен скрипт `start.sh` произойдет перекомлиляция файла `dist/main.js`.
Для отображения изменений просто перегрузите WEB-страницу.
