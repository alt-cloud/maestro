...
В целом, архитектура безопасности реализована на высоком уровне. Применяется подход "защита в глубине" (defense in depth).

| Тип атаки | Статус защиты | Реализация в коде |
| :--- | :--- | :--- |
| **XSS (Cross-Site Scripting)** | ✅ **Отлично** | Заголовки `Content-Security-Policy` (`default-src 'none'`, `object-src 'none'`) полностью запрещают выполнение скриптов. Заголовок `X-Content-Type-Options: nosniff` отключает MIME-sniffing, не давая браузеру интерпретировать JSON как HTML. API всегда отдает строгий `application/json`. |
| **CSRF (Cross-Site Request Forgery)** | ✅ **Отлично** | Аутентификация требует кастомного заголовка `X-API-Key` или `Authorization: Bearer`. Браузеры не могут отправить такие заголовки cross-origin без preflight `OPTIONS` запроса, который жестко контролируется `flask-cors`. Сервер **не принимает** ключи через URL-параметры (`?api_key=`), что закрывает классические векторы CSRF. Сессионные куки настроены как `SameSite=Strict`. |
| **Clickjacking** | ✅ **Отлично** | Дублирующая защита: устаревший заголовок `X-Frame-Options: DENY` и современная директива CSP `frame-ancestors 'none'`. |
| **CORS** | ✅ **Отлично** | Используется `flask-cors` со строгим списком разрешенных источников (`origins`). Использование wildcard `*` запрещено и вызывает падение приложения на старте. Настроено кэширование preflight-запросов (`max_age=3600`). |
| **XS-Leaks** | ✅ **Отлично** | Внедрены современные заголовки изоляции контекста браузера: `Cross-Origin-Opener-Policy`, `Cross-Origin-Embedder-Policy` и `Cross-Origin-Resource-Policy`. |
| **postMessage** | ⚠️ **Фронтенд-уровень** | API не использует `postMessage`, но защищен от смежных атак (например, перехвата данных из iframe). Бэкенд никогда не возвращает HTML или JSONP (игнорирует `?callback=`), отдавая только чистый JSON. |
| **IP Spoofing / Несанкционированный доступ** | ✅ **Отлично** | Реализован `MAESTRO_API_WHITELIST`. Сервер проверяет IP клиента (с учетом `ProxyFix` и `X-Forwarded-For`) по маске подсети. Если IP не в белом списке, запрос блокируется (403), даже при наличии валидного API-ключа. |
