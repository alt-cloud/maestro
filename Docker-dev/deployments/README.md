...
**В переменной `MAESTRO_API_KEYS` необходимо при развертывании указать свои ключи**. 
Длина ключа должна бы не меньше 16 символов. Рекомендуется для генерации использовать команды `pwgen`, `apg` и другиею
Если ключ имеет поле `expires_at` необходимо по истечении указанного времени  обновлять значение ключа и поле `expires_at`.
