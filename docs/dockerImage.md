...
`root.crt`  в каталог `/etc/pki/ca-trust/source/anchors/caddy-local.crt` компьютеров, с на которых запускается браузер клиента.
...
```
Так как `caddy` генерирует самоподписный сертификат при первом обращении к https-странице боаузер предупредит, что серимфмкаи является самоподписным и необходимо подьвердить, что вы планируете с ним работать.
